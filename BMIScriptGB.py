...
def delayed_accuracy(y_true, y_pred): # This is to delay the accuracy score
    time.sleep(1)  # delay so my CPU doesn't cook itself
...
gb_param_grid = {
    ...
    "learning_rate":[0.01],#,0.1, 0.5, 1.0], # took too long to run this these and it was .96 accurate so I just didn't test them
    "subsample": [0.3],#, 0.5, 1.0, 2.0],
    "min_samples_split": [2],#, 5, 10, 20],
    "max_features": ['log2'],#, 'sqrt'],
    "min_impurity_decrease": [ 0.3]#, 0.1, 0.08]
    }

# Grid search using 6-fold cross validation
gb_grid_search = GridSearchCV(estimator=gb, param_grid=gb_param_grid, n_jobs = -1, cv = 6, verbose=3,scoring=delayed_scorer)
...
gb_grid_search.fit(x_train_s, y_train_s)
...
yscore_raw = gb_best_grid.predict_proba(X_test)
...
#create ROC curve
plt.plot(fpr, tpr, label='%s ROC (area = %0.3f)' % ('Gradient Boosting', roc_auc))
plt.plot([0, 1], [0, 1],'r--')
plt.xlim([0.0, 1.0])
plt.ylim([0.0, 1.05])
...
plt.savefig('Gradient Boosting_MOF.png')

# print the best hyperparameters
print(f'The best parameters are {gb_best_params}')

# The best parameters are {'learning_rate': 0.01, 'max_depth': 50, 'max_features': 'log2', 'min_impurity_decrease': 0.3, 'min_samples_split': 2, 'n_estimators': 50, 'subsample': 0.3}

# Predict the labels of the test set: y_pred
y_hat = gb_best_grid.predict(X_test)
...
result = permutation_importance(gb_best_grid, x_train_s, y_train_s, n_repeats=4, random_state=seed)
